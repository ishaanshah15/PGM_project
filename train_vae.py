...
import sys
from action_dataset import get_action_vae_dataloader
import sys
sys.path.append('vae')
from model import ActionAEModel
...
def ae_loss(model, x):
    ...
    loss = torch.sum(torch.pow(x_hat - x,2))/x.shape[0]
...
def vae_loss(model, x, beta = 1):
    ...
    std_normal = torch.normal(torch.zeros_like(mu),torch.ones_like(mu))

    z = std_normal*torch.exp(log_std) + mu

    x_hat = model.decoder(z) 

    #recon_loss = F.cross_entropy(out)
    #kl_loss = 

    recon_loss = torch.sum(torch.pow(x_hat - x,2))/x.shape[0]

    kl_loss = -2*log_std + torch.exp(2*log_std) + mu**2 - 1
    kl_loss = 0.5*torch.sum(kl_loss)/x.shape[0]
...
def linear_beta_scheduler(max_epochs=None, target_val = 1):
    ...
    """
    def _helper(epoch):
        denom = max(max_epochs-1,1)
        return (float(epoch)/denom)*target_val
    return _helper
